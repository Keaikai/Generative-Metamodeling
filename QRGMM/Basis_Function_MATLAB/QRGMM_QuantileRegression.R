...
#setwd("D:/QRGMM/Esophageal_Cancer_Simulator/Treatment_Effects")
...
dir.create("./data/QRGMMcoeff1", recursive = TRUE, showWarnings = FALSE)
dir.create("./data/QRGMMcoeff2", recursive = TRUE, showWarnings = FALSE)
...
for (runi in 1:run) {
  ...
  path <- paste("./data/ECtraindata1/ECtraindata1_", runi, sep = "")
  ...
  path <- paste("./data/QRGMMcoeff1/QRGMMcoeff1_", runi, sep = "")
  path <- paste(path, ".csv", sep = "")
...
for (runi in 1:run) {
  ...
  path <- paste("./data/ECtraindata2/ECtraindata2_", runi, sep = "")
  path <- paste(path, ".csv", sep = "")
  GMMtraindata <- read.csv(file = path, sep = ",", header = FALSE)
  GMMtraindata <- data.matrix(GMMtraindata)
  ...
  path <- paste("./data/QRGMMcoeff2/QRGMMcoeff2_", runi, sep = "")
  path <- paste(path, ".csv", sep = "")
  write.csv(coeff, path)
}
