...
def generate(x_test, data, args, model_name, sample_num=1):
    """
    Input:  x_test: numpy (N, input_dim)
    Output: y_gen:
            - if sample_num == 1: numpy (N, output_dim)  (denormalized)
            - if sample_num > 1 : numpy (N, output_dim, sample_num) (denormalized)
    """
    ...
    # Reshape back if multiple samples
    if sample_num > 1:
        # (N*sample_num, output_dim) -> (N, output_dim, sample_num)
        y_pred = y_pred.view(test_dim, y_pred.shape[-1], -1)

    # Denormalize (IMPORTANT for output_dim > 1)
    y_np = y_pred.detach().cpu().numpy()

    # data.y_mean, data.y_std should be shape (output_dim,)
